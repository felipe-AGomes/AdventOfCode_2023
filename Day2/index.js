const input = `Game 1: 2 green, 12 blue; 6 red, 6 blue; 8 blue, 5 green, 5 red; 5 green, 13 blue; 3 green, 7 red, 10 blue; 13 blue, 8 red
...
class RuleCheck {
	...
	arrayGames;
	rule;
	constructor(gameInput, rule) {
		this.rule = rule;
		this.gameInput = gameInput;

		this.arrayGames = this.separateSets(gameInput);
	}

	separateSets(game) {
		return game.split('\n').map((game) => {
			const sets = game.match(/: (.*)/)[1].split(';');
			return {
				[game.match(/[0-9]+/)[0]]: sets.map((set) => {
					return {
						r: (set.match(/([0-9]+) r/) || [,])[1],
						g: (set.match(/([0-9]+) g/) || [,])[1],
						b: (set.match(/([0-9]+) b/) || [,])[1],
	...
	check() {
		if (!this.rule) {
			throw new Error('Regra não definida');
		}
		return this.arrayGames.reduce((acc, currentGame, i) => {
			let isAproved = true;
			currentGame[i + 1].forEach((set) => {
				if (set.r > this.rule.r || set.g > this.rule.g || set.b > this.rule.b) {
					isAproved = false;
				}
			});

			if (isAproved) {
				acc.push(Object.keys(currentGame)[0]);
			}
	...
	checkMaxColorByGame() {
		return this.arrayGames.reduce((acc, currentGame, i) => {
			acc.push(
				currentGame[i + 1].reduce(
					(acc, currentSet) => {
						acc.r = +currentSet.r > acc.r ? currentSet.r : acc.r;
						acc.g = +currentSet.g > acc.g ? currentSet.g : acc.g;
						acc.b = +currentSet.b > acc.b ? currentSet.b : acc.b;
...
const ruleCheck = new RuleCheck(input, { r: 12, g: 13, b: 14 });

console.log(ruleCheck.check().reduce((c, a) => +c + +a, 0));

console.log(
	ruleCheck.checkMaxColorByGame().reduce((c, a) => a.r * a.g * a.b + c, 0),
);
