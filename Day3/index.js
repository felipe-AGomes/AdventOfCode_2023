const input = `.........798...145.........629....579.....455.....................130.............243.................154........167........................
...
class EngineMaintenance {
	...
	checkPieces() {
		...
		for (let i = 0; i < this.pieceMatrix.length; i++) {
			const pieceLine = this.pieceMatrix[i];
			for (let j = 0; j < pieceLine.length; j++) {
				const piece = pieceLine[j];
				const regex = /\W/;

				if (regex.test(piece) && piece !== '.') {
					const currentGrearRatio = [];
					directions.forEach(({ row, col }) => {
						if (this.isNumberAtLineColumn(i + row, j + col)) {
							const number = this.getNumberAtLineColumn(i + row, j + col);
							if (number !== piecesNumber[piecesNumber.length - 1]) {
								piecesNumber.push(number);

								if (piece == '*') {
									currentGrearRatio.push(number);
								}
							}
						} else {
							piecesNumber.push('.');
					...
					if (currentGrearRatio.length === 2) {
						gearRatio.push(currentGrearRatio);
					}
				}
			}
		}
		...
		return {
			sumPieces: piecesNumber
				.filter((piece) => !isNaN(piece))
				.reduce((acc, c) => acc + Number(c), 0),
			gearRatio: gearRatio.reduce((acc, curr) => acc + +curr[0] * +curr[1], 0),
		};
	}

	isNumberAtLineColumn(line, column) {
		return !isNaN(this.pieceMatrix[line][column]);
	}

	getNumberAtLineColumn(line, column) {
		let isNumber = true;
		let maxNumber = '';
		let count = column;
		while (isNumber) {
			if (!isNaN(this.pieceMatrix[line][count])) {
				maxNumber = this.pieceMatrix[line][count] + maxNumber;
				count--;
			} else {
				isNumber = false;
			}
		...
		while (isNumber) {
			if (!isNaN(this.pieceMatrix[line][count])) {
				maxNumber = maxNumber + this.pieceMatrix[line][count];
				count++;
			} else {
				isNumber = false;
			}
...
const engineMaintenance = new EngineMaintenance(input);

console.log(engineMaintenance.checkPieces());
