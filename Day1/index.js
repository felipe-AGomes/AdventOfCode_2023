...
function findSequence(input) {
	function replaceStringToNumber(input) {
		function replaceAllOcurrency(input, search, newVal) {
			let count = 0;
			let lastFind = 0;
			while (count < input.length) {
				if (input.indexOf(search, lastFind) >= 0) {
					lastFind = input.indexOf(search, lastFind) + 1;
					const firstSlice = input.slice(0, lastFind);
					const secondSlice = input.slice(lastFind);

					input = firstSlice + newVal + secondSlice;
				}
				count++;
			}
			return input;
		...
		return input.map((line) => {
			line = replaceAllOcurrency(line, 'one', '1');
			line = replaceAllOcurrency(line, 'two', '2');
			line = replaceAllOcurrency(line, 'three', '3');
			line = replaceAllOcurrency(line, 'four', '4');
			line = replaceAllOcurrency(line, 'five', '5');
			line = replaceAllOcurrency(line, 'six', '6');
			line = replaceAllOcurrency(line, 'seven', '7');
			line = replaceAllOcurrency(line, 'eight', '8');
			line = replaceAllOcurrency(line, 'nine', '9');

			return line;
		});
	}

	input = replaceStringToNumber(input);
	...
	const result = input.map((line) => {
		...
		while (first == null && count <= line.length) {
			if (!isNaN(line[count])) {
				first = line[count];
				count = line.length - 1;
			} else {
...
console.log(findSequence(lines));

console.log(findSequence(lines).reduce((n, c) => +n + +c, 0));
